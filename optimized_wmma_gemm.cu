...
__global__ void wmmaKernel(const half *A, const half *B, float *C) {
  ...
  // Within the block, launch 8 warps -> 256 threads.
  // Each warp computes a 16-row & 128-column strip of the full 128x128 tile, in
  ...
  // Each warp computes 8 WMMA tiles along the column direction, so we need
  // an array of accumulator fragments: one for each of the 8 sub-tiles in this
  // warp's row.
  wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, float> acc_frag[8];
  ...
  for (int k_tile = 0; k_tile < K_DIM / WMMA_K; k_tile++) {
    ...
    // Now, loop over the 8 sub-tiles in columns that this warp is responsible for
    #pragma unroll
    for (int i = 0; i < 8; i++) {
  ...
  // Because the accumulation is done over K, each warp stores its 8 resulting
  // 16x16 tiles
  #pragma unroll
  for (int i = 0; i < 8; i++) {
...
int main() {
  ...
  // Grid:
  //    One block per 128x128 tile of C ==> (16384/128) x (16384/128) = (128,
  //    128)
  // Block:
  //    Eight warps per block -> 256 threads
